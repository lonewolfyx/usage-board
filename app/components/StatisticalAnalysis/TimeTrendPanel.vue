<template>
    <StatisticalAnalysisPanel
        :description="`${rangeLabel} spend heatmap. Darker cells mean higher daily spend.`"
        ...
        <div class="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-3">
            ...
            >
                ...
                <div class="mt-1 flex items-center justify-between gap-2">
                    <span class="text-sm font-semibold">{{ formatCurrency(item.costUSD) }}</span>
                    <span class="text-xs text-muted-foreground">{{ formatCompactNumber(item.totalTokens) }}</span>
                </div>
            </div>
        </div>
    </StatisticalAnalysisPanel>
...
import type { DailyTokenUsage } from '~/composables/useUsageDashboard'
import { computed } from 'vue'
import { formatCompactNumber, formatCurrency } from '~/composables/useUsageDashboard'

defineOptions({
    name: 'StatisticalAnalysisTimeTrendPanel',
})
...
function getHeatmapLevel(costUSD: number, maxCost: number) {
    if (costUSD <= 0 || maxCost <= 0)
        return 0

    return Math.min(9, Math.max(1, Math.ceil((costUSD / maxCost) * 9)))
}
...
const trendItems = computed(() => {
    ...
    const maxCost = Math.max(...yearItems.map(item => item.usage?.costUSD ?? 0))
    ...
    return yearItems.map((item) => {
        ...
        return {
            ...
            colorClass: heatmapLevels[getHeatmapLevel(costUSD, maxCost)],
...
const heatmapCells = computed(() => {
    ...
    const days = trendItems.value.map((item, index) => ({
        ...
        title: `${item.date}: ${formatCurrency(item.costUSD)} / ${formatCompactNumber(item.totalTokens)} tokens`,
    }))
